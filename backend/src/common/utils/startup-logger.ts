...
export class StartupLogger {
  ...
  static logStartup(config: StartupConfig): void {
    ...
    // App Header
    this.logCentered(
      `${this.ICONS.server} ${config.appName}`,
      boxWidth,
    ...
    // Environment Info
    console.log(
      this.colorize('║', this.COLORS.cyan) +
        ' ' +
        this.colorize('Environment:', this.COLORS.bold),
    );
    this.logInfo(
      'NODE_ENV',
      config.environment,
      this.getEnvColor(config.environment),
    );
    ...
    if (config.swaggerEnabled) {
      ...
      console.log(
        this.colorize('║', this.COLORS.cyan) +
          ' ' +
          this.colorize(
            `${this.ICONS.swagger} Swagger Documentation:`,
            this.COLORS.bold,
          ),
      );
    ...
    console.log(
      this.colorize('║', this.COLORS.cyan) +
        ' ' +
        this.colorize('Service Status:', this.COLORS.bold),
    );
    console.log(
      this.colorize('║', this.COLORS.cyan) +
        ' ' +
        this.colorize('─'.repeat(boxWidth - 2), this.COLORS.dim),
    );
    ...
    console.log(
      this.colorize('  ' + successMsg, this.COLORS.bold + this.COLORS.green),
    );
    console.log('\n');
  }
  ...
  static logStartupError(error: Error, context?: string): void {
    ...
    console.log(
      this.colorize('║', this.COLORS.red) +
        this.colorize(
          '          ❌ APPLICATION STARTUP FAILED'.padEnd(boxWidth) + '║',
          this.COLORS.bold + this.COLORS.red,
        ),
    );
    console.log(this.colorize('╚' + border + '╝', this.COLORS.red));
    console.log('');
  ...
  private static logCentered(text: string, width: number, color: string): void {
    const padding = Math.max(
      0,
      Math.floor((width - this.stripAnsi(text).length) / 2),
    );
    const paddedText = ' '.repeat(padding) + text;
    console.log(
      this.colorize('║', color) +
        this.colorize(paddedText, color) +
        this.colorize(' ║', color),
    );
  }
  ...
  private static logInfo(
    ...
    console.log(
      line.replace(
        new RegExp(`${this.escapeRegExp(value)}$`),
        this.colorize(value, valueColor),
      ),
    );
  }
